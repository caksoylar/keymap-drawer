...
class KanataKeymapParser(KeymapParser):
    ...
    def _parse_cfg(self, cfg_str: str) -> list[str | pp.ParseResults]:
        return (pp.nested_expr("(", ")")[1, ...]
            .ignore(";;" + pp.SkipTo(pp.lineEnd))
            .ignore("#|" + pp.SkipTo("|#"))
            .parse_string(cfg_str))
    ...
    def _parse(self, in_str: str, file_name: str | None = None) -> tuple[dict, KeymapData]:
        ...
        """
        nodes = self._parse_cfg(in_str)
        # print(nodes, sep="\n")
        defsrc = next(node[1:] for node in nodes if node[0] == "defsrc")
        # print(defsrc)

        try:
            defalias = next(node[1:] for node in nodes if node[0] == "defalias")
