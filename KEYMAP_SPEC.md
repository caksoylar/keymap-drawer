...
- **`layout_name`** (equivalent to `-l`/`--layout-name` on the command line):
  ...
  node label that the keyboard's physical layout definition contains.
  If this isn't specified, the first physical layout node in `dts_layout` file or the first entry in the mapping value is used.
